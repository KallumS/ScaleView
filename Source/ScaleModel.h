...
inline const std::map<std::string, int> coreRank {
    ...
    // so it sits just below it exactly as min7b5 sits just below min7.
    // Leaving it out of the vocabulary cost real names - it fell through to
    // rankUnnamed, and C Eb Gb B read Baddb9/C: a major triad with a
    // flattened ninth bolted on and no seventh in it at all.
    { "min/b/maj7",  13 },
